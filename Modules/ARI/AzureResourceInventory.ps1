...
param ($TenantID,
    ...
    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Debbuging Mode: On. ErrorActionPreference was set to "Continue", every error will be presented.')
    ...
    Function Variables {
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Cleaning default variables')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking if -Online parameter will have to be forced.')
        if(!$Online.IsPresent)
            {
                ...
                if([string]::IsNullOrEmpty($LocalFilesValidation))
                    {
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Using -Online by force.')
    ...
    Function Extractor {
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Extractor function')

        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Powershell Edition: ' + ([string]$psversiontable.psEdition))
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Powershell Version: ' + ([string]$psversiontable.psVersion))
        function checkAzCli() {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting checkAzCli function')
            Write-Host "Validating Az Cli.."
            $azcli = az --version
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Current az cli version: ' + $azcli[0])
            if ($null -eq $azcli) {
            ...
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Current Resource-Graph Extension Version: ' + $azcliExt.version)
            ...
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'ImportExcel Module Version: ' + ([string]$VarExcel.Version.Major + '.' + [string]$VarExcel.Version.Minor + '.' + [string]$VarExcel.Version.Build))
            if ($null -eq $VarExcel) {
            ...
            $VarExcel = Get-InstalledModule -Name ImportExcel -ErrorAction silentlycontinue
            if ($null -eq $VarExcel) {
                Read-Host 'Administrator rights required to install ImportExcel Module. Press <Enter> to finish script'
                Exit
            }
        }

        function LoginSession() {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting LoginSession function')
            ...
            if (!$TenantID) {
                ...
                write-host ""
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Cleaning az account cache')
                az account clear | Out-Null
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Calling az login')
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking number of Tenants')
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Extracting Subscription details')
        ...
        function checkPS() {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting checkPS function')
            ...
            if ($CShell) {
                ...
                $Global:DefaultPath = $FolderPath
                $Global:DiagramCache = $FolderPath + '\DiagramCache\'
                $Global:Subscriptions = az account list --output json --only-show-errors | ConvertFrom-Json
                if ($SubscriptionID)
                    {
                        if($SubscriptionID.count -gt 1)
        ...
        if ($IncludeTags.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+"Tags will be included")
            $GraphQueryTags = ",tags "
        } else {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+"Tags will be ignored")
            $GraphQueryTags = ""
        }
        ...
        if (![string]::IsNullOrEmpty($ManagementGroup))
            {
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Management group name supplied: ' + $ManagmentGroupName)
                ...
                {
                    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Management groups found: ' + $group.count)
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Number of Subscriptions Found: ' + $SubCount)
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking report folder: ' + $DefaultPath )
    ...
    $Global:ExtractionRuntime = Measure-Command -Expression {
        ...
        if(![string]::IsNullOrEmpty($ResourceGroup) -and [string]::IsNullOrEmpty($SubscriptionID))
            {
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Resource Group Name present, but missing Subscription ID.')
                Write-Host ''
        ...
        else
            {
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Resources')
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Network Resources')
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Backup Resources')
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for AVD Resources')
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Resource Containers')
                ...
                if (!($SkipPolicy.IsPresent)) 
                    {
                        ...
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Policies Resources')
                ...
                if (!($SkipAdvisory.IsPresent)) 
                    {
                        ...
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Advisories')
                ...
                if ($SecurityCenter.IsPresent) 
                    {
                        ...
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Invoking Inventory Loop for Security Resources')
    ...
    Function RunMain {
        ...
        $Global:File = ($DefaultPath + $Global:ReportName + "_Report_" + (get-date -Format "dd-MM-yyyy_HH_mm") + ".xlsx")
        #$Global:DFile = ($DefaultPath + $Global:ReportName + "_Diagram_" + (get-date -Format "dd-MM-yyyy_HH_mm") + ".vsdx")
        $Global:DDFile = ($DefaultPath + $Global:ReportName + "_Diagram_" + (get-date -Format "dd-MM-yyyy_HH_mm") + ".xml")
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Excel file:' + $File)

        #### Generic Conditional Text rules, Excel style specifications for the spreadsheets and tables:
        $Global:TableStyle = "Light19"
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Excel Table Style used: ' + $TableStyle)
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Importing List of Unsupported Versions.')
        If ($RunOnline -eq $true) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/Support.json')
            $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/Support.json')
        }
        Else {
            if($PSScriptRoot -like '*\*')
                {
                    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\Support.json')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking if Draw.io Diagram Job Should be Run.')
        if (!$SkipDiagram.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Draw.io Diagram Processing Job.')
            Start-job -Name 'DrawDiagram' -ScriptBlock {
                ...
                Add-Content -Path $Logfile -Value ('DrawIOCoreJob - '+(get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - Starting Draw.IO Job')

                If ($($args[8]) -eq $true) {
                    Add-Content -Path $Logfile -Value ('DrawIOCoreJob - '+(get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - Running Online')
                    $ModuSeq = (New-Object System.Net.WebClient).DownloadString($($args[10]) + '/Extras/DrawIODiagram.ps1')
                }
                Else {
                    if($($args[0]) -like '*\*')
                        {
                            Add-Content -Path $Logfile -Value ('DrawIOCoreJob - '+(get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - Running Local')
                ...
                Add-Content -Path $Logfile -Value ('DrawIOCoreJob - '+(get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - Calling Draw.IO Thread')
                try
                    {
                    ...
                    {
                        Add-Content -Path $Logfile -Value ('DrawIOCoreJob - '+(get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+$_.Exception.Message)
                    }
                Add-Content -Path $Logfile -Value ('DrawIOCoreJob - '+(get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - Draw.IO Ended.')
        ...
        <######################################################### VISIO DIAGRAM JOB ######################################################################>
        <#
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking if Visio Diagram Job Should be Run.')
        if ($Diagram.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Visio Diagram Processing Job.')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking If Should Run Security Center Job.')
        if ($SecurityCenter.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Security Job.')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking If Should Run Policy Job.')
        if (!$SkipPolicy.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Policy Processing Job.')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking If Should Run Advisory Job.')
        if (!$SkipAdvisory.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Advisory Processing Job.')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Subscriptions job.')
        ...
        switch ($Resources.count) 
            ...
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Processing Jobs in '+ $DebugEnvSize +' Mode.')
            ...
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Total Resources Being Processed: '+ $ResourcesCount)
            ...
            while ($Looper -lt $Loop) {
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Resources Being Processed in ResourceJob_'+ $LoopCountStr + ': ' + $ResourceCount)
                ...
                if($DebugEnvSize -in ('Large','Enormous') -and $JobLoop -eq 5)
                    {
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Waiting Batch of Jobs to Complete.')
                        ...
                        while (get-job -Name $InterJobNames | Where-Object { $_.State -eq 'Running' }) {
                            ...
                            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'initial Jobs Running: '+[string]($jb | Where-Object { $_.State -eq 'Running' }).count)
                            $c = [math]::Round($coun)
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Jobs Collector.')
        ...
        while (get-job -Name $JobNames | Where-Object { $_.State -eq 'Running' }) {
            ...
            $c = (((($jb.count - ($jb | Where-Object { $_.State -eq 'Running' }).Count)) / $jb.Count) * 100)
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Jobs Still Running: '+[string]($jb | Where-Object { $_.State -eq 'Running' }).count)
            $c = [math]::Round($c)
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Jobs Compleated.')
        ...
        Foreach ($Job in $JobNames)
            {
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Job '+ $Job +' Returned: ' + ($TempJob.values | Where-Object {$_ -ne $null}).Count + ' Resource Types.')
                $Global:SmaResources += $TempJob
            }

        <############################################################## REPORTING ###################################################################>

        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Starting Reporting Phase.')
        ...
        Else {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Running Offline, Gathering List Of Modules.')
            if($PSScriptRoot -like '*\*')
                {
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Modules Found: ' + $Modules.Count)
        $Lops = $Modules.count
        $ReportCounter = 0
        ...
        foreach ($Module in $Modules) {
            ...
            if ($ModuleResourceCount -gt 0)
                {
                    Start-Sleep -Milliseconds 100
                    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+"Running Module: '$ModuleName'. Resources Count: $ModuleResourceCount")
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Resource Reporting Phase Done.')
        ...
        if($QuotaUsage.IsPresent)
            {
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Generating Quota Usage sheet for: ' + $Global:AzQuota.count + ' Subscriptions/Regions.')
                ...
                If ($RunOnline -eq $true) {
                    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/QuotaUsage.ps1')
                    $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/QuotaUsage.ps1')
                }
                Else {
                    if($PSScriptRoot -like '*\*')
                        {
                            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\QuotaUsage.ps1')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking if Should Generate Security Center Sheet.')
        if ($SecurityCenter.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Generating Security Center Sheet.')
            ...
            If ($RunOnline -eq $true) {
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/SecurityCenter.ps1')
                $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/SecurityCenter.ps1')
            }
            Else {
                if($PSScriptRoot -like '*\*')
                    {
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\SecurityCenter.ps1')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking if Should Generate Policy Sheet.')
        if (!$SkipPolicy.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Generating Policy Sheet.')
            ...
            while (get-job -Name 'Policy' | Where-Object { $_.State -eq 'Running' }) {
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Policy Job is: '+(get-job -Name 'Policy').State)
                Start-Sleep -Seconds 2
            }
            ...
            If ($RunOnline -eq $true) {
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/Policy.ps1')
                $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/Policy.ps1')
            }
            Else {
                if($PSScriptRoot -like '*\*')
                    {
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\Policy.ps1')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Checking if Should Generate Advisory Sheet.')
        if (!$SkipAdvisory.IsPresent) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Generating Advisor Sheet.')
            ...
            while (get-job -Name 'Advisory' | Where-Object { $_.State -eq 'Running' }) {
                ...
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Advisory Job is: '+(get-job -Name 'Advisory').State)
                Start-Sleep -Seconds 2
            }
            ...
            If ($RunOnline -eq $true) {
                Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/Advisory.ps1')
                $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/Advisory.ps1')
            }
            Else {
                if($PSScriptRoot -like '*\*')
                    {
                        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\Advisory.ps1')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Generating Subscription sheet for: ' + $Subscriptions.count + ' Subscriptions.')
        ...
        If ($RunOnline -eq $true) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/Subscriptions.ps1')
            $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/Subscriptions.ps1')
        }
        Else {
            if($PSScriptRoot -like '*\*')
                {
                    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\Subscriptions.ps1')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Generating Overview sheet (Charts).')
        ...
        If ($RunOnline -eq $true) {
            Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$RawRepo + '/Extras/Charts.ps1')
            $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/Charts.ps1')
        }
        Else {
            if($PSScriptRoot -like '*\*')
                {
                    Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Looking for the following file: '+$PSScriptRoot + '\Extras\Charts.ps1')
        ...
        Write-Debug ((get-date -Format 'dd-MM-yyyy_HH_mm_ss')+' - '+'Finished Charts Phase.')
